...
const CreateSequence = () => {
  ...
  const removeStep = (i: number) =>
    setSeqSteps((p) => p.filter((_, idx) => idx !== i));
  ...
  const handleSubmit = async () => {
    try {
      if (!form.name) return alert("Sequence name required");

      const templates = seqSteps
        .map((s) => s.templateName)
        .filter(Boolean);

      const delays = seqSteps.map(
        (s) => Number(s.day) * 24 * 60 * 60
      );

      const cleanRecipients = recipients.filter((r) => r.phone);

      if (templates.length === 0) {
        return alert("Add at least one step");
      }
      ...
      alert(err.response?.data?.message || "Failed to create sequence.");
    }
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold text-foreground">
          Create New WhatsApp Sequence
        </h1>
        <div className="flex items-center gap-4">
          <StepBar steps={steps} current={step} />
          <button className="border border-crm-border rounded-lg px-4 py-2 text-sm text-foreground hover:bg-muted">
            Save as Draft
          </button>
        </div>
      </div>
      ...
      <div className="bg-card rounded-xl border border-crm-border p-8 min-h-[400px]">
        ...
        {step === 0 && (
          <div>
            <input
              className="border rounded-lg px-4 py-2.5 text-sm w-full mb-5"
            ...
            <div className="flex justify-end gap-3 mt-8">
              <button onClick={() => navigate("/sequences")} className="border px-4 py-2 text-sm">
                Cancel
              </button>
              <button onClick={() => setStep(1)} className="bg-blue-600 text-white px-5 py-2 rounded-lg text-sm">
                Save & Continue
              </button>
            </div>
          </div>
        ...
        {step === 1 && (
          <div>
            <button onClick={() => setShowAI(true)} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm mb-4 flex items-center gap-2">
            ...
            {seqSteps.map((s, i) => (
              <div key={i} className="flex gap-2 mb-2">
                <input
                  ...
                  onChange={(e) => {
                    const ns = [...seqSteps];
                    ns[i].templateName = e.target.value;
                    setSeqSteps(ns);
                  }}
                />
                <input
        ...
        {step === 2 && (
          <div>
            ...
            <button onClick={() => setRecipients((p) => [...p, { name: "", phone: "" }])}>
              + Add Recipient
            </button>
        ...
        {step === 3 && (
          <div>
            <div className="flex justify-end gap-3 mt-8">
              ...
              <button onClick={handleSubmit} className="bg-blue-600 text-white px-5 py-2 rounded-lg">
                Submit
              </button>
            </div>
          </div>

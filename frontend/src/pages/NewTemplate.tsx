...
const NewTemplate = () => {
  ...
  // 🔥 AI Generate (only if backend supports it)
  const handleGenerate = async () => {
    try {
      setGenerating(true);
  ...
  // 🔥 Save Template (CONNECTED TO BACKEND)
  const handleSave = async () => {
    try {
      if (!name || !body) {
        return alert("Name and Body are required");
      }
  ...
  return (
    <div>
      ...
      <button
        ...
        className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm mb-4 flex items-center gap-2"
      >
        <Sparkles size={16} />
        Generate with AI
      </button>
      ...
      {/* AI Modal */}
      {showAI && (
        <Modal title="Generate with AI" onClose={() => setShowAI(false)}>
          <textarea
            rows={3}

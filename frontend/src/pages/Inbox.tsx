...
//       {/* LEFT PANEL SAME */}
//       <div className="w-[220px] bg-card border-r border-crm-border flex flex-col">
//         <div className="p-3 border-b">All Chats</div>
//       </div>
...
//           <span className="font-semibold">Kate Morrison</span>
//         </div>
...
//               <div>
//                 <div
//                   className={`px-3.5 py-2.5 text-sm max-w-[320px] ${
//                     msg.type === "sent"
//                       ? "bg-crm-blue-light rounded-xl rounded-br-sm"
//                       : "bg-card border rounded-xl rounded-bl-sm"
//                   }`}
//                 >
//                   {msg.text}
//                 </div>
//                 <div className="text-[11px] mt-1">{msg.time}</div>
//               </div>
//             </div>
//           ))}
...
//         {/* INPUT (FIXED 🔥) */}
//         <div className="bg-card border-t p-3 flex items-center gap-3">
...
//             onClick={sendMessage}
//             disabled={loading}
//             className="w-9 h-9 rounded-full bg-crm-blue flex items-center justify-center"
//           >
...
//       {/* RIGHT PANEL SAME */}
//       <div className="w-[220px] bg-card border-l p-4">
//         Contact Details
//       </div>
//     </div>
//   );
...
const Inbox = () => {
  ...
  // ✅ SEND MESSAGE (YOU SEND TO CUSTOMER)
  const sendMessage = () => {
  ...
  // 🤖 AI ASSIST (IMPROVE YOUR MESSAGE)
  const improveWithAI = async () => {
    ...
    try {
      const res = await api.post("/chat", {
        message: `Improve this reply professionally: ${input}`,
      });

      setInput(res.data.reply); // 👈 AI replaces your text
    } catch (err: any) {
    ...
    <div className="flex h-[calc(100vh-56px)] -m-6">
      ...
      <div className="flex-1 flex flex-col bg-background">
        ...
        <div className="bg-card border-b border-crm-border p-3 flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-muted" />
          <div>
            <span className="font-semibold text-sm">Kate Morrison</span>
              ...
              <div>
                <div
                  className={`px-3.5 py-2.5 text-sm max-w-[320px] ${
                    msg.type === "sent"
                      ? "bg-crm-blue-light rounded-xl rounded-br-sm"
                      : "bg-card border border-crm-border rounded-xl rounded-bl-sm"
                  }`}
                >
                  {msg.text}
                </div>
        ...
        <div className="bg-card border-t border-crm-border p-3 flex items-center gap-3">
          ...
          <input
            className="flex-1 border border-crm-border rounded-lg px-4 py-2 text-sm"
          ...
          <button
            onClick={sendMessage}
            className="w-9 h-9 rounded-full bg-crm-blue flex items-center justify-center hover:bg-crm-blue-dark"
          >
            <Send size={16} className="text-white" />
          </button>
        </div>
      ...
      {/* RIGHT PANEL */}
      <div className="w-[220px] bg-card border-l border-crm-border p-4">
        <div className="font-semibold text-sm mb-2">Contact Details</div>
        <div className="text-xs text-gray-500">Kate Morrison</div>
        <div className="mt-2 text-xs">+116355204421</div>
        <div className="text-xs">demo@gmail.com</div>
      </div>
    </div>
  );
};
